...
export default function AdminUsersPage() {
  ...
  const handleReissueNft = async (
    targetUserId: number
  ) => {
    const ok = confirm(
      "このユーザーに初期NFTを再付与しますか？"
    );
    ...
    try {
      ...
      console.error("初期NFTの再付与に失敗しました:", error);
      alert("初期NFTの再付与に失敗しました");
    }
  };

  const handleResendMetadata = async (
    targetUserId: number
  ) => {
    const ok = confirm(
      "このユーザーのMetadataを再送信しますか？"
    );
    ...
    try {
      const res = await fetch(
        "/api/admin/users/resend-metadata",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteUser = async (
    ...
    const ok = confirm(
      `${targetUserName}さんを削除しますか？\n\n` +
        ...
        "・研究室評価\n" +
        "・投票、チャットなどの関連情報\n\n" +
        "ブロックチェーンおよびIPFS上に記録済みの情報は削除できません。\n\n" +
        "この操作は元に戻せません。"
    );
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        {users.length === 0 ? (
          <section style={styles.noUsersCard}>
            登録ユーザーはいません。
          </section>
        ) : (
                ...
                <section style={styles.deleteSection}>
                  <div>
                    ...
                    <p style={styles.deleteDescription}>
                      サーバー上のユーザー情報と関連データを削除します。ブロックチェーンおよびIPFS上の記録は削除されません。
                    </p>
                  </div>
